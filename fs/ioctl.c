...
extern int tty_ioctl(int dev, int cmd, int arg);		// chr_dev/tty_ioctl.c

typedef int (*ioctl_ptr)(int dev,int cmd,int arg);		// 定义输入输出控制(ioctl)函数指针类型
// 取系统中设备种类的宏
...
{	
	...
	// 否则就取对应文件的属性，并据此判断文件的类型。如果该文件既不是字符设备文件，也不
