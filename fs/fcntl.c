...
static int dupfd(unsigned int fd, unsigned int arg)
{
	...
	// 找到的新句柄值rg大于最多打开文件数（即没有空闲项），则返回出错码并退出。
	while (arg < NR_OPEN)
		if (current->filp[arg])
			arg++;
		else
	...
	// 即在运行exec类函数时，不会关闭用dup创建的句柄。并令该文件结构指针等于原句
...
// 复削文件句柄系统调用。
// 复制指定文件句柄o1dfd,新句柄的值是当前最小的未用句柄值。
// 参数：fi1des-被复制的文件句柄。
// 返回新文件句柄值。
...
// 同的命令有不同的含义。
// 对于复制句柄命令F_DUPFD,arg是新文件句柄可取的最小值
// 对于设置文件操作和访问标志命令F_SETFL,arg是新的文件操作和访问模式
// 对于文件上锁命令F_GETLK、F_SETLK和F_SETLKW,arg是指向flock结构的指针。但本内核中没有实现文件上锁功能。
// 返回：若出错，则所有操作都返回-1。若成功，那么F_DUPFD返回新文件句柄：F_GETFD
// 返回文件句柄的当前执行时关闭标志close_on_exec;F_GETFL返回文件操作和访间标志。
int sys_fcntl(unsigned int fd, unsigned int cmd, unsigned long arg)
{	
	struct file * filp;

	// 首先检查给出的文件句柄的有效性。然后根据不同命令cmd进行分别处理
	// 如果文件句柄值大于一个进程最多打开文件数NR_OPEN,或者该句柄的文件结构指针为空，则返回出错码并退出。
	...
	switch (cmd) {
		...
		case F_GETFL:				// 取文件状态标志和访问模式
			return filp->f_flags;
		case F_SETFL:				// 设置文件状态和访问模式(根据arg设置添加非阻塞标志)
			filp->f_flags &= ~(O_APPEND | O_NONBLOCK);
			filp->f_flags |= arg & (O_APPEND | O_NONBLOCK);
			return 0;
		case F_GETLK:	case F_SETLK:	case F_SETLKW://文件锁未实现
			return -1;
		default:
			return -1;
	}
