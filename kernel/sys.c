...
// 设置当前任务的实际以及/或者有效组ID(gd)。如果任务没有超级用户特权，那么只能互
// 换其实际组ID和有效组ID。如果任务具有超级用户特权，就能任意设置有效的和实际的组
// ID。保留的gid(saved gid)被设置成与有效gid同值。
...
// 设置进程组号(gid)。如果任务没有超级用户特权，它可以使用setgid将其有效gid
...
// 返回从1970年1月1日00：00：00GT开始计时的时间值（秒）。如果tloc不为null,则时间值也存储在那里。
// 由于参数是一个指针，而其所指位置在用户空间，因此需要使用函数put_fs_long来
 ...
 * or vice versa.
 */
// 无特权的用户可以将实际的用户标识符(real uid)改成有效的用户标识符(effective uid),反之亦然。
// 设置任务的实际以及/或者有效的用户ID(uid)。如果任务没有超级用户特权，那么只能
// 互换其实际的uid和有效的uid。如果任务具有超级用户特权，就能任意设置有效的和实
// 际的用户ID。保留的uid(saved uid)被设置成与有效uid同值。
...
// 设置系统开机时间。参数tptr是从1970年1月1日00：00：00GT开始计时的时间值（秒）。
// 调用进程必须具有超级用户权限。其中HZ100,是内核系统运行烦率。
// 由于参数是一个指针，而其所指位置在用户空间，因此需要使用函数get_fs_long来访问该
// 值。在进入内核中运行时，段寄存器fs被默认地指向当前用户数据空间。因此该函数就可利
// 用fs来访问用户空间中的值。
// 函数参数提供的当前时间值减去系统已经运行的时间秒值(jiffies/H忆)即是开机时间秒值。
...
// 获取当前任务运行时间统计值。tms结构中包括进程用户运行时间、内核（系统）时间、子进
...
// 情况下，参数pgid指定了要加入的现有进程组ID,此时该组的会话ID必须与将要加入进
// 程的相同
int sys_setpgid(int pid, int pgid)
{
	int i;

	// 如果参数pid=O,则使用当前进程号。如果pgid为O,则使用当前进程pid作为pgid。
	if (!pid)
		pid = current->pid;
	if (!pgid)
		pgid = current->pid;

	// 扫描任务数组，查找指定进程号pid的任务。如果找到了进程号是pid的进程，那么若该
	// 任务已经是会话首领，则出错返回。若该任务的会话ID与当前进程的不同，则也出错返回。
	// 否则设置进程的pgrp=pgid,并返回0。若没有找到指定pid的进程，则返回进程不存在
	// 出错码
...
int sys_setsid(void)
{
	...
	// 首领(leader=1),并且设置当前进程会话号session和组号pgrp都等于进程号pid,
...
// 该结构定义在include/sys/utsname.h文件中。
