...
#include <stdarg.h>		// 标准参数头文件。以宏的形式定义变量参数列表。主要说明了1个类型(va_list)和三个宏(va_start,va_arg和va_end),用于
						// vsprintf、vprintf、vfprintf函数。
#include <string.h>		// 字符串头文件,定义了一些有关字符串操作的嵌入函数
...
#define SPACE	8		/* space if plus 				如果是+，则放置空格*/
	...
	,int type)
{
	...
	if (num==0)
		...
		tmp[i++]=digits[do_div(num,base)]; //num每次都在减小  十进制32转16进制  tmp = [0, 2], 后面转成字符串即tmp倒序 0x20
	...
	// 若类型指出是特殊转换，则对于八进制转换结果头一位放置一个0：而对于十六进制则存放0x
	if (type&SPECIAL)
		if (base==8)
			*str++ = '0';
		else if (base==16) {
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
	...
	int precision;		/* min. # of digits for integers; max		// min 整数数字个数，max，字符串中字符个数
	...
	for (str=buf ; *fmt ; ++fmt) {
		...
		// 格式转换指示字符串均以'%开始，这里从t格式字符串中扫描%'，寻找格式转换字符串的开始。
		...
		// 下面取得格式指示字符串中的标志域，并将标志常量放入f1gs变量中。
		flags = 0;
		repeat:
		...
		if (is_digit(*fmt))
			...
			/* it's the next argument */   // bugfix ++fmt
		...
		// 若格式转换指示是x或X,则表示对应参数需要打印成十六进制数输出。’x’表示用小写字母表示。
		case 'x':
			flags |= SMALL;
		case 'X':
		...
		// 如果格式转换字符是d或i u,则表示对应参数是整数，d，i代表符号整数，因此需要加上
		// 带符号标志。u代表无符号整数。
		case 'd':
		case 'i':
			flags |= SIGN;
		...
		// 若格式转换指示符是n，则表示要把到 目前为止转换输出字符数(cnt) 保存到对应参数指针指定的位置中。
		// 首先利用varg取得该参数指针，然后将已经转换好的字符数存入该指针所指的位置。
		case 'n':
			ip = va_arg(args, int *);
			*ip = (str - buf);
			break;
